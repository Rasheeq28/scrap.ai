...
# CRITICAL: Ensure the project root is in the path BEFORE other imports
    ...
    if st.button("🔍 Discover New Properties", help="Fetch property IDs and slugs from API", use_container_width=True):
        with st.spinner("Discovering from API..."):
            ...
            if isinstance(results, dict):
                ...
                st.info(f"✨ New in DB: **{results.get('new_in_db', 0)}**")
                if results.get('new_items_api', 0) > 0:
                    st.toast(f"New Listings Added: +{results['new_items_api']}")
                if results.get('new_pages_api', 0) > 0:
                    st.toast(f"New Pages Added: +{results['new_pages_api']}")
            elif results is True:
                st.success("Discovery complete! (Reloading UI symbols...)")
                st.rerun()
            else:
                st.error("Discovery failed or returned unexpected data. Please restart Streamlit.")
